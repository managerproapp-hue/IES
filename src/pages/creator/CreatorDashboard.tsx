...
import { SignatureIcon, TrashIcon, WarningIcon, DownloadIcon, DemoIcon } from '../../components/icons';
    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-6">
          <Card title="Modo Demo" icon={<DemoIcon className="w-8 h-8 text-blue-500"/>} className="border border-blue-500">
             <p className="mb-4">Carga un conjunto completo de datos de demostración para exhibir todas las funcionalidades de la aplicación. <strong className="text-yellow-500">¡Atención!</strong> Esto reemplazará todos los datos actuales.</p>
             <button onClick={() => setIsDemoModalOpen(true)} className="w-full bg-blue-600 text-white py-2 px-4 rounded-md hover:bg-blue-700 no-print">Cargar Datos de Demostración</button>
          </Card>
      ...
      <Modal isOpen={isDemoModalOpen} onClose={() => setIsDemoModalOpen(false)} title="Confirmar Carga de Demo">
        <div>
            <div className="text-center">
                <WarningIcon className="w-16 h-16 text-blue-500 mx-auto"/>
                <p className="text-lg font-semibold my-4">¿Cargar datos de demostración?</p>
                <p className="text-gray-500">Esta acción <strong className="text-red-500">sobrescribirá todos los datos actuales</strong> con un conjunto de datos predefinido para la demostración. La aplicación se recargará después.</p>
            </div>
            <div className="mt-6 flex justify-end space-x-3">
              <button onClick={() => setIsDemoModalOpen(false)} className="px-4 py-2 bg-gray-200 dark:bg-gray-600 rounded-md hover:bg-gray-300">Cancelar</button>
              <button onClick={handleLoadDemoData} className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">Sí, cargar demo</button>
            </div>
        </div>
      </Modal>
