...
import { Company, Event, Incident, Order, Product, Supplier, User, ReceptionItem, Recipe } from '../types';
...
export const generateRecipePdf = (recipe: Recipe, authorName: string, productsMap: Map<string, Product>, appName?: string) => {
    const doc = new jsPDF();
    ...
    doc.setFontSize(10);
    doc.text(`Por: ${authorName}`, 14, y);
    y += 10;

    if (recipe.photo) {
        try {
            const imgData = recipe.photo;
    ...
    doc.text(splitDescription, 14, y + 5);
    ...
    (doc as any).autoTable({
        ...
        body: [
            ...
            ['Coste / Ración', `${recipe.cost.toFixed(2)} €`],
            ['Precio Venta', `${recipe.price.toFixed(2)} €`],
        ],
    ...
    const ingredientsBody = recipe.ingredients.map(ing => {
        const product = productsMap.get(ing.productId);
        return [product ? product.name : 'Desconocido', ing.quantity, ing.unit];
    });
    ...
    y += 5;
    doc.setFontSize(10);
    const splitSteps = doc.splitTextToSize(recipe.preparationSteps, 180);
    doc.text(splitSteps, 14, y);
    y += (splitSteps.length * 5) + 10;

    if (recipe.keyPoints) {
        if (y > pageHeight - 30) { doc.addPage(); y = 20; }
        doc.setFontSize(14);
        doc.text('Puntos Clave', 14, y);
        y += 5;
        doc.setFontSize(10);
        const splitKeyPoints = doc.splitTextToSize(recipe.keyPoints, 180);
        doc.text(splitKeyPoints, 14, y);
    }
    ...
    doc.save(`receta_${recipe.name.replace(/\s/g, '_')}.pdf`);
};