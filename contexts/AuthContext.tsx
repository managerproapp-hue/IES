...
// Fix: Explicitly type AuthProvider as a React.FC with a required children prop.
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const [simulatedRole, setSimulatedRoleState] = useState<SimulatedRole | null>(() => storage.session.get('simulatedRole', null));
  ...
  const setActiveRole = (role: Role) => {
    if (currentUser?.roles.includes(role)) {
      ...
      if (role !== Role.STUDENT) {
        setSimulatedRole(null);
        storage.session.remove('simulatedRole');
      }
    }
  };
  ...
  const setSimulatedRole = (role: SimulatedRole | null) => {
    ...
    if (role) {
      storage.session.set('simulatedRole', role);
    } else {
      storage.session.remove('simulatedRole');
    }
  };
