...
const UserFormModal: React.FC<UserFormModalProps> = ({ isOpen, onClose, onSave, userToEdit, personnelMode = false }) => {
  ...
  const availableRoles = personnelMode 
    ? allRoles.filter(r => [Role.ADMIN, Role.TEACHER, Role.MANAGER].includes(r))
    : allRoles.filter(r => r !== Role.CREATOR);
  ...
  useEffect(() => {
    if (isOpen) {
        if (userToEdit) {
            ...
            setSelectedRoles(personnelMode ? [] : [Role.STUDENT]);
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={userToEdit ? 'Editar Usuario' : (personnelMode ? 'Añadir Personal' : 'Añadir Usuario')}>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {userToEdit && userToEdit.roles.includes(Role.TEACHER) && (
             <div className="p-3 bg-gray-100 dark:bg-gray-900 rounded-md">
                <h4 className="text-sm font-semibold text-gray-800 dark:text-gray-200">Módulos Académicos Asignados</h4>
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 italic">(Funcionalidad futura) Aquí se mostrará la lista de módulos que imparte este profesor.</p>
             </div>
        )}
