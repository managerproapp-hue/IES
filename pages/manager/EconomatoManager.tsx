...
import { Order, OrderStatus, Incident, Supplier, Event, Product, ReceptionLineStatus, ReceptionItem } from '../../types';
...
export const EconomatoManager: React.FC = () => {
    ...
    const handleReceivedQuantityChange = (productId: string, value: string) => {
        ...
        setReceptionItems(prev => {
            // FIX: Explicitly type Map to ensure correct type inference for Map.get().
            const newMap = new Map<string, ReceptionItem>(prev);
    ...
    const handleDownloadSheet = () => {
        ...
        const receptionData = aggregatedProductsForSelectedEvent.map(({ product }) => ({
            product,
            receptionInfo: receptionItems.get(product.id)! as ReceptionItem
        }));
        
        const incidentsForEvent = incidents.filter(inc => 
            receptionData.some(d => d.product.id === inc.productId && d.receptionInfo.status === 'incidencia')
        );
