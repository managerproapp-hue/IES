...
const ComposeMessageModal: React.FC<ComposeMessageModalProps> = ({ isOpen, onClose }) => {
    ...
    const { users, classrooms, sendMessage } = useData();
    ...
    // Fix: Add an explicit return type to the `useMemo` hook to help TypeScript correctly infer the types.
    const { availableRecipients, groupShortcuts } = useMemo<{
        ...
        if (activeRole === Role.STUDENT) {
            ...
            if (myClassroom) {
                ...
                const classmates = myClassroom.students.filter(s => s.id !== currentUser.id) as User[]; // Cast to User
                const recipients = [...(tutor ? [tutor] : []), ...classmates];
                return { 
                    availableRecipients: recipients, 
                    groupShortcuts: {
                        'Mi Tutor': tutor ? [tutor.id] : [],
                        'Toda la Clase': classmates.map(s => s.id)
                    } 
                };
            }
            return { availableRecipients: [], groupShortcuts: {} };
        }
        
        // Logic for Admin, Teacher, Manager, Creator
    ...
    }, [currentUser, activeRole, users, classrooms]);
                         ...
                         <div className="absolute z-10 mt-1 w-full bg-white dark:bg-gray-800 border dark:border-gray-700 rounded-md shadow-lg max-h-60 overflow-y-auto">
                            {Object.entries(groupShortcuts).length > 0 && (
                                <div className="p-2 border-b dark:border-gray-700">
                                    ...
                                    {/* FIX: Explicitly type the destructured arguments from Object.entries to ensure `ids` is correctly typed as `string[]`. */}
