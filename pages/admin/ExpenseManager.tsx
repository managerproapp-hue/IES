            ...
            <div className="space-y-6">
                ...
                <Card title="Gasto por Proveedor (Estimado)">
                    ...
                    <table className="w-full text-sm">
                        ...
                        <tbody>
                            {Object.entries(analysisData.costBySupplier).map(([id, cost]) => (
                                <tr key={id} className="border-b dark:border-gray-700"><td className="p-2">{suppliers.find(s=>s.id === id)?.name}</td><td className="p-2">{/* FIX: Cast `cost` to number because Object.entries can produce a value of type `any` or `unknown` in some TS configurations. */}
{formatCurrency(cost as number)}</td></tr>
                            ))}
                        </tbody>
                    </table>
                </Card>
