...
import { User, Profile, getProfileDisplayName } from '../../types';
...
export const UserManager: React.FC = () => {
    ...
    const filteredUsers = useMemo(() => {
        ...
        return users.filter(u => u.name.toLowerCase().includes(filter.toLowerCase()) || u.email.toLowerCase().includes(filter.toLowerCase()));
    }, [users, filter]);
                        ...
                        <tbody>
                            {filteredUsers.map(user => (
                                <tr key={user.id} className="border-b dark:border-gray-700">
                                    ...
                                    {/* FIX: Correctly call getProfileDisplayName within the map function to avoid passing the index as the context. */}
...
const UserFormModal: React.FC<{ user: User | null, onClose: () => void, onSave: (user: User) => void }> = ({ user, onClose, onSave }) => {
    ...
    const isSuperUser = user?.email === 'all@example.com';
    ...
    return (
        <Modal isOpen={true} onClose={onClose} title={user ? "Editar Usuario" : "Nuevo Usuario"}>
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {isSuperUser ? (
                    <div>
                        <label>Perfiles (Super Usuario)</label>
                    ...
                    <div>
                        <label>Perfil</label>
                        <select 
                            value={formState.profiles[0] || ''} 
                            onChange={e => setFormState({ ...formState, profiles: e.target.value ? [e.target.value as Profile] : [] })}
                            required
                            className="w-full mt-1 p-2 border rounded dark:bg-gray-700"
                        >
                            <option value="">-- Seleccionar Perfil --</option>
                            {Object.values(Profile).map(p => (
                                <option key={p} value={p}>{getProfileDisplayName(p)}</option>
                            ))}
                        </select>
                    </div>
                )}
