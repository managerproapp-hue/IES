...
import { Recipe, RecipeIngredient, Role } from '../../types';
import ConfirmationModal from '../../components/ConfirmationModal';
import { EditIcon, TrashIcon, DownloadIcon, UploadIcon, Share2Icon, PlusCircleIcon } from '../../components/icons';

const MyRecipes: React.FC = () => {
    const { recipes, addRecipe, updateRecipe, deleteRecipe, products, users, classrooms } = useData();
    ...
    const pathPrefix = activeRole === Role.STUDENT ? '/student' : '/teacher';
    ...
    const { myRecipes, publicRecipes } = useMemo(() => {
        ...
        let filteredPublicRecipes: Recipe[];
        if (activeRole === Role.STUDENT) {
            const myClassroom = classrooms.find(c => c.students.some(s => s.id === currentUser.id));
            const classmateIds = myClassroom ? myClassroom.students.map(s => s.id) : [];
            filteredPublicRecipes = recipes.filter(r => r.isPublic && r.creatorId !== currentUser.id && classmateIds.includes(r.creatorId) && searchFilter(r));
        } else {
            filteredPublicRecipes = recipes.filter(r => r.isPublic && r.creatorId !== currentUser.id && searchFilter(r));
        }

        return { myRecipes: filteredMyRecipes, publicRecipes: filteredPublicRecipes };
    }, [recipes, currentUser, searchTerm, productMap, activeRole, classrooms]);
    ...
    const handleExportSingle = (recipe: Recipe) => {
        ...
        link.href = url;
        link.download = `receta_${recipe.name.replace(/\s+/g, '_')}.recipe`;
    ...
    const handleExportAll = () => {
        ...
        const link = document.createElement('a');
        link.href = url;
        link.download = 'mi-recetario.recipe';
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        URL.revokeObjectURL(url);
            ...
            try {
                ...
                recipesToImport.forEach((importedRecipe: any) => {
                    ...
                    const newIngredients: RecipeIngredient[] = importedRecipe.ingredients.map((ing: any) => {
                        const product = products.find(p => p.id === ing.productId || (ing.productName && p.name.toLowerCase() === ing.productName.toLowerCase()));
                        if (!product) notFoundIngredients.add(ing.productName || 'Ingrediente desconocido');
                        return { productId: product?.id || ing.productId, quantity: ing.quantity || 0, unit: ing.unit || 'uds' };
                    });
                ...
                if (notFoundIngredients.size > 0) summary += `\n\nIngredientes no encontrados en el catálogo local: ${Array.from(notFoundIngredients).join(', ')}. Por favor, revísalos manualmente.`;
                alert(summary);

            } catch (error) {
                alert('Error al leer el archivo. Asegúrate de que es un archivo .recipe válido.');
            } finally {
        ...
        <div className="space-y-8">
            <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button onClick={handleImportClick} className="btn-secondary whitespace-nowrap"><UploadIcon className="w-5 h-5 mr-2"/> Importar</button>
                        ...
                        <div key={recipe.id} className="card">
                            ...
                            <div className="p-5 flex-grow">
                                ...
                                <div className="card-content">
                                    <h4>Ingredientes:</h4>
                                    <ul>{recipe.ingredients.slice(0, 3).map((ing, i) => <li key={i}>{productMap.get(ing.productId) || 'Desconocido'}</li>)}</ul>
                                </div>
                            </div>
                            <div className="card-footer">
                                <button onClick={() => handleExportSingle(recipe)} className="btn-action" title="Descargar"><DownloadIcon className="w-5 h-5"/></button>
            ...
            <input type="file" ref={fileInputRef} onChange={handleFileSelected} accept=".recipe" className="hidden" />
