...
// FIX: Added 'Event' to the import to resolve type collision with the global Event type.
import { Classroom, User, Profile, getProfileDisplayName, AppData, Product, Supplier, Order, Event } from '../../../types';
...
const CatalogManagerTab: React.FC<{ classroom: Classroom }> = ({ classroom }) => {
    ...
    const handleSaveProduct = (data: Partial<Product>) => {
        if(selectedProduct) {
            setProducts(products.map(p => p.id === selectedProduct.id ? {...p, ...data} as Product : p));
        } else {
    ...
    const handleSaveSupplier = (data: Partial<Supplier>) => {
         if(selectedSupplier) {
            setSuppliers(suppliers.map(s => s.id === selectedSupplier.id ? {...s, ...data} as Supplier : s));
        } else {
